...
test("ThreatFox validates supplied IOC semantics",()=>{const record=normalizeThreatFox({id:"41",ioc:"203.0.113.42:443",ioc_type:"ip:port",threat_type:"botnet_cc",malware_printable:"Example",confidence_level:75,first_seen:"2026-08-21 00:00:00 UTC",last_seen:null,tags:["c2"]},at);assert.equal(record?.kind,"ipv4");assert.equal(record?.confidence,75);assert.equal(record?.lastSeen,undefined);const invalid=normalizeThreatFox({id:"42",ioc:"not:an:address",ioc_type:"ipv6",first_seen:"2026-08-21 00:00:00 UTC"},at);assert.equal(invalid?.kind,"infrastructure")});
...
test("MalwareBazaar is metadata-only and validates SHA-256",()=>{const hash="a".repeat(64);const record=normalizeMalwareBazaar({sha256_hash:hash,sha1_hash:"b".repeat(40),md5_hash:"c".repeat(32),first_seen:"2026-08-21 00:00:00",file_type:"exe",file_size:42,signature:"Example"},at);assert.equal(record?.indicator,hash);assert.equal(record?.metadata.fileSize,42);assert.equal("download" in (record?.metadata??{}),false);assert.equal(normalizeMalwareBazaar({sha256_hash:"bad",first_seen:"2026-08-21"},at),null)});
